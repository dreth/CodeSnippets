...
def simulate_curves(functions, reward=1e6, sim_accs=1e3, min_eth_amount=0.1, max_eth_amount=1e3, swise_price=swise_price, eth_price=eth_price, amounts_apply=lambda x: x, mult_swise=True):
    ...
    amounts = amounts_apply(np.linspace(min_eth_amount, max_eth_amount, int(sim_accs)))
    ...
    for name, function in functions.items():
        ...
        sample_data[f'new_monthly_apr{funcname}'] = (swise_price*sample_data[f'new_monthly_reward{funcname}'])/value

        # yearly APR after apply
        sample_data[f'new_yearly_apr{funcname}'] = (
            sample_data[f'new_monthly_reward{funcname}']*12)/value
...
def plot_simulation(df, fontsize=35, figsize=(20,40), linewidth=4, x_axis=3*['amounts'], y_axis=['monthly_reward', 'monthly_apr', 'yearly_apr'], usd_or_swise='SWISE', savefig=True):
    ...
    for n in range(len(y_axis)):
        ...
        # plot
        ax[n].plot(filt_df.loc[:,f'original_{y_axis[n]}'], label=f'Original {y_axis_labels[n]}', linewidth=linewidth)
        ax[n].plot(filt_df.loc[:,f'new_{y_axis[n]}'], label=f'New {y_axis_labels[n]}', linewidth=linewidth)
...
#function itself
def quantile_flatten(col, functions=qtys*[np.power], quantiles=np.linspace(0.01,1,qtys), powers=np.linspace(10,100,qtys)):
    ...
    for i,q in enumerate(quantiles):
        if i == 0:
            ...
            new_col_section = col[(col >= np.quantile(col, quantiles[i-1])) & (col <= np.quantile(col, q))]
        arrays.append(functions[i](new_col_section,quantiles[::-1][i]))
...
df = simulate_curves(functions=functions, reward=6e6, amounts_apply=lambda x: x, mult_swise=False)
